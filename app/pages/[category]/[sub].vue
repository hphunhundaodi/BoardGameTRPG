<template>
  <div class="PageSubCategoryRoot min-h-screen bg-gradient-to-br from-slate-900 via-purple-900/50 to-slate-900 p-6">
    <div class="relative max-w-4xl mx-auto">
      ...
      <AccordionGameList :games="games" :category="sub" />
...
const games = computed<GameDetails[]>(() => {
  const main = category.value as keyof typeof gameDetailsData
  const subKey = sub.value as keyof (typeof gameDetailsData)[typeof main]
  const list = gameDetailsData[main]?.[subKey]
  return Array.isArray(list) ? (list as GameDetails[]) : []
})
